...
- DO, after every `coder` or `devops` node returns `ok`, automatically queue a `security-auditor` node with the changed paths. If its `output.blocking === true`, queue a `coder` node to fix the highest-severity blocking findings, then re-queue `security-auditor`; loop until clean OR the security retry count exceeds 3, then return `status:"blocked"` with the blocker in `summary`. The daemon independently enforces this gate before completing a root task.
